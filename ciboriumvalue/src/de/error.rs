...
impl<T> From<ciborium_llvalue::Error<T>> for Error<T> {
    #[inline]
    fn from(value: ciborium_llvalue::Error<T>) -> Self {
        match value {
            ciborium_llvalue::Error::Io(x) => Self::Io(x),
            ciborium_llvalue::Error::Syntax(x) => Self::Syntax(x),
        }
    }
}
